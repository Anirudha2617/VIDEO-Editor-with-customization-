...
import { Download, Undo2, Redo2, Save, X, Sparkles, FolderOpen } from 'lucide-react';
...
export function EditorPage() {
    ...
    const handleGroupSelected = () => {
        ...
        const newClips = clips.map(c => selectedClipIds.includes(c.id) ? { ...c, groupId: gid } : c);
        updateClips(newClips);
    };

    const handleUngroupSelected = () => {
        if (selectedClipIds.length === 0) return;
        const newClips = clips.map(c => selectedClipIds.includes(c.id) ? { ...c, groupId: null } : c);
        updateClips(newClips);
    };
    ...
    const handleDropAsset = (assetId: string, trackId: string, time: number) => {
        ...
        // Phase 3: Try Unified Library Pipeline First
        const libraryClip = libraryPipeline ? libraryPipeline.createClipFromItem(assetId, trackId, time) : null;
        if (libraryClip) {
            console.log('[App] Created clip from Library Pipeline:', libraryClip);
            globalCommandManager.execute(new AddClipCommand(commandContext, libraryClip as Clip));
            setSelectedClipIds([libraryClip.id]);
            setCurrentTime(time);
            return;
        }

        // Fallback to Legacy Asset Logic
    ...
    const libraryPipeline = createLibraryPipeline();
